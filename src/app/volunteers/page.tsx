  ...
  return (
    ...
    >
      <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-5 mb-6 flex items-center gap-6 flex-wrap shadow-sm">
        <div>
          <div className="text-sm font-bold text-[var(--muted)]">إجمالي المتطوعين</div>
      ...
      <section className="mb-8 rounded-xl border border-[var(--border)] bg-[var(--card)] p-5 shadow-sm">
        <h2 className="text-lg font-extrabold mb-4 text-[var(--heading)]">إضافة متطوع</h2>
        <form action={addVolunteer} className="grid grid-cols-2 gap-3">
          ...
          <input
            name="skills"
            placeholder="المهارات (مثال: تصوير، تنظيم، سائق)"
            className="rounded-lg border border-[var(--border)] px-3.5 py-2.5 text-[15px] bg-[var(--bg)] focus:border-[var(--brand-blue)] focus:outline-none"
          />
          <input
            name="availability"
            placeholder="التوفر (مثال: عطلة الأسبوع فقط)"
            className="rounded-lg border border-[var(--border)] px-3.5 py-2.5 text-[15px] bg-[var(--bg)] focus:border-[var(--brand-blue)] focus:outline-none"
          />
          <input
            name="notes"
      ...
      <ListSearch scopeId="volunteers-list" placeholder="بحث بالاسم، الهاتف، البريد، الجماعة، المهارات..." />
      <div id="volunteers-list" className="grid md:grid-cols-2 gap-4">
        ...
        {filtered.length === 0 && (
          <p className="text-[15px] text-[var(--muted)] md:col-span-2">ماكاينش متطوعون يطابقو هاد الفلترة.</p>
        )}
      </div>
    </PageShell>
  );
