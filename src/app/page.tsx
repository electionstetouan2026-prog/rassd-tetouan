  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <StatCard label={dict.home.statCommunes} value={allCommunes.length} accent="var(--brand-blue)" />
        <StatCard label={dict.home.statStations} value={realStations.length} accent="var(--accent-teal)" />
        <StatCard label={dict.home.statCovered} value={coveredReal} accent="var(--severity-neutral)" />
        <StatCard label={dict.home.statObservers} value={allObservers.length} accent="var(--severity-medium)" />
      </div>

      <h2 className="text-xl font-extrabold mb-4 text-[var(--heading)]">{dict.home.observersStatusTitle}</h2>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
        {STATUS_ORDER.map((status) => (
          ...
          >
            ...
            <div className="text-sm font-bold text-[var(--muted)] mt-1.5">{STATUS_LABEL[status]}</div>
          </div>
        ))}
      </div>
